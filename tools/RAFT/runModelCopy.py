...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for imfile1, imfile2 in zip(images[:-1], images[1:]):
            ...
            file = open("/home/cse/segmentationTest/raftFlowResults_walking_static/" + imfile2[-21:-4] + "_flow","w")
            
            file.write(str(np.median(nflow_up)) + " " + str(np.median(nflow_up_a)))
            file.write("\n")
            print("writing to " + "/home/cse/segmentationTest/raftFlowResults_walking_static/" + imfile2[-21:-4] + "_flow" )
            for j in range(480):
                for k in range(640):
